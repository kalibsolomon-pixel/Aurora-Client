...
public final class PaletteEngine {
    ...
    /**
     * WCAG relative luminance of an ARGB color (alpha ignored). Enough for a
     * robust light-vs-dark text choice without a full WCAG pass.
     */
    ...
    public static int pickOnColor(int bgArgb, float hue) {
        int light = hsl(hue, 0.05f, 0.97f);
        int dark  = hsl(hue, 0.10f, 0.08f);
        return contrastRatio(bgArgb, light) >= contrastRatio(bgArgb, dark) ? light : dark;
    }
    ...
    private static double channelLuminance(int c) {
        double v = (c & 0xFF) / 255.0;
        return v <= 0.03928 ? v / 12.92 : Math.pow((v + 0.055) / 1.055, 2.4);
    }

    /** Convert opaque RGB to {@code [h(0..360), s(0..1), l(0..1)]}. */
    ...
    /** Build an opaque ARGB color from HSL. */
